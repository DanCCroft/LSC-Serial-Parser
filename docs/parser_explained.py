...
1.  Sample runs
    ...
    - 2 lines of data: Header line + variable-length data line
...
Calibration data is identified and passed through unchanged, preserving the
...
# Look for a valid SNC header anywhere in the preamble
for line in lines:
    if any(line.startswith(h) for h in SNC_HEADERS):
        first_line = line
        break

# Fallback: if no header found, use the first non-empty line
if not first_line and lines:
    first_line = lines[0]

print(f"FIRST LINE (resolved): {first_line!r}")

# Optional visibility: notify if preamble was skipped
if lines and lines[0] != first_line:
    print(f"NOTICE: Skipped preamble line(s): {lines[0]!r}")

# NOTE:
# Instrument output may include preamble lines (e.g., power failure logs)
# before the actual report header. Therefore, we cannot assume the first
# line is the true header — we scan for a valid header instead.
